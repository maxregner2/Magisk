#!/usr/bin/env python3
import argpars#!/usr/bin/env python3
import argparse
import glob
import lzma
...
def dump_flag_header():
    flag_txt = textwrap.dedent(
        ...
        import urllib.request
        # Define constants
        os_name = "linux" if sys.platform.startswith("linux") else "darwin"
        ndk_root = os.path.expanduser("~/.magisk/ndk")
        ndk_path = os.path.join(ndk_root, "ondk")
        adb_path = os.path.join(os.path.expanduser("~/.magisk"), "adb")
        gradlew = "./gradlew" if sys.platform.startswith("linux") else "gradlew.bat"
        support_targets = {"native", "cpp", "rust", "java"}
        default_targets = {"native", "cpp", "rust"}
        ...
        def run_ndk_build(flag):
            # Run ndk-build command
            execv(["ndk-build", "-j", "8", flag])
        ...
        def cleanup(args):
            ...
            if "cpp" in args.target:
                header("* Cleaning C++")
                rm_rf(os.path.join("native", "libs"))
                rm_rf(os.path.join("native", "obj"))
                rm_rf(os.path.join("native", "out"))

            if "rust" in args.target:
                header("* Cleaning Rust")
                rm_rf(os.path.join("native", "src", "target"))
                rm(os.path.join("native", "src", "boot", "proto", "mod.rs"))
                rm(os.path.join("native", "src", "boot", "proto", "update_metadata.rs"))
            ...
            if "java" in args.target:
                ...
                rm_rf(os.path.join("app", "src", "debug"))
                rm_rf(os.path.join("app", "src", "release"))
        ...
        def setup_ndk(args):
            ...
            for target in ["arm-linux-androideabi", "i686-linux-android"]:
                ...
                rm(os.path.join(src_dir, ".DS_Store"))
        ...
        def patch_avd_ramdisk(args):
            ...
            ini = os.path.join(os.path.dirname(args.ramdisk), "advancedFeatures.ini")
